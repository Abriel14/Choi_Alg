...
//--------------------------------------------------------
  ...
  class Cl_Arbre{ 
    private: 
      ...
      void Cl_Arbre<T>::Vers_vector(Cl_Noeud<T> *,vector<T> *);   
...
//------------------------------------------------------------------------------
// FONCTION      : Export
// DESCRIPTION   : Export vers un vector
//-------------------------------------------------------------------------------
  template <class T> 
  void Cl_Arbre<T>::Export(vector<T> *list){ 
 //Si racine différent de NULL alors il affiche l'arbre
   if(racine!=NULL) 
   //Affectation du pointeur racine à pNoeud
     Vers_vector( racine, list);
    else
...
//------------------------------------------------------------------------------
// FONCTION      : Vers_vector
// DESCRIPTION   : fonction recursive qui renvoie un liste des éléments
//-------------------------------------------------------------------------------
  template <class T> 
  void Cl_Arbre<T>::Vers_vector(Cl_Noeud<T> *pNoeud,vector<T> *list){ 

   //On traverse vers la gauche (valeurs inferieures), si il existe 
     if (pNoeud->gauche) Vers_vector(pNoeud->gauche); 

   //On affiche le noeud en cours 
     if (pNoeud) list.push_back(pNoeud->valeur); 

   //Et traverse vers la droite (valeurs superieurs) 
     if (pNoeud->droite) Vers_vector(pNoeud->droite); 
